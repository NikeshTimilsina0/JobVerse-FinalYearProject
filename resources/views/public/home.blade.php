...
<header class="py-5 bg-white border-bottom text-center">
    <div class="container py-4">
        <span class="badge bg-primary-subtle text-primary border border-primary-subtle px-3 py-2 rounded-pill fw-bold text-uppercase small mb-3">Verified Employment Hub</span>
        ...
        <form action="{{ route('jobs.index') }}" method="GET" class="row g-2 justify-content-center max-width-md mx-auto bg-light p-3 border rounded shadow-sm">
            <div class="col-md-5">
            ...
            <div class="col-md-3 d-grid">
                <button type="submit" class="btn btn-primary fw-bold">Find Jobs</button>
            </div>
        </form>
    </div>
</header>
            ...
            <div class="col-md-4">
                <div class="card bg-white border h-100 p-4 shadow-sm d-flex flex-column justify-content-between">
                    <div>
                        ...
                        <p class="text-secondary small mb-0 text-truncate-3">{{ Str::limit($job->description, 140) }}</p>
                    </div>
                    <div class="pt-3 border-top mt-3 d-flex justify-content-between align-items-center">
                        <span class="fw-bold text-primary small">{{ $job->salary_range ?? 'Negotiable' }}</span>
                        <a href="{{ route('jobs.show', $job->id) }}" class="btn btn-sm btn-outline-primary fw-semibold">View Details</a>
                    </div>
                </div>
            </div>
        @empty
